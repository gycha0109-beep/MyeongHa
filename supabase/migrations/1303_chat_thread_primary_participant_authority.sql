-- Tighten owner-scoped Chat thread runtime binding so browser/API callers never
-- manufacture a primary Character by taking the first active participant.
-- An active single- or multi-Character thread must expose exactly one active
-- primary participant in its active content bundle before any participant order
-- can become Character Room authority.
...
'Owner-scoped active Chat thread runtime binding. Requires exactly one active primary Character in the active content bundle before returning deterministic participants.';
