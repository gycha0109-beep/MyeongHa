-- Forward-only replay hardening for already server-verified provider payment evidence.
-- A fresh verification invocation naturally has a fresh verifiedAt provenance timestamp.
-- Replay classification therefore compares stable verified facts while preserving the
-- immutable first-write verifiedAt stored on the canonical Receipt/Event payload.
...
begin
  ...
  if v_receipt.id is not null or v_event.id is not null then
    if v_receipt.id is null or v_event.id is null
       ...
       or not (v_receipt.verified_payload_jsonb ? 'verifiedAt')
       or not (v_event.verified_payload_jsonb ? 'verifiedAt')
       or v_receipt.verified_payload_jsonb ->> 'verifiedAt' is distinct from pg_catalog.to_char(
         v_receipt.verified_at at time zone 'UTC',
         'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'
       )
       or v_receipt.verified_payload_jsonb ->> 'verifiedAt' is distinct from v_event.verified_payload_jsonb ->> 'verifiedAt'
       or (v_receipt.verified_payload_jsonb - 'verifiedAt') is distinct from (v_payload - 'verifiedAt')
       ...
       or (v_event.verified_payload_jsonb - 'verifiedAt') is distinct from (v_payload - 'verifiedAt')
...
comment on function public.cmd_persist_verified_payment_evidence_v1(
  ...
  'Atomic VerifiedCommerceEvidenceV2 persistence: immutable first-write verifiedAt plus stable-fact replay across fresh verification invocation timestamps. No entitlement effect.';
