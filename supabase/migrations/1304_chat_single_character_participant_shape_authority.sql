-- Preserve the source-backed single-Character thread shape at the runtime read
-- authority boundary. A single-Character thread is not allowed to expose an
-- additional active Character merely because one valid primary also exists.
...
begin
  ...
  if v_thread.thread_type = 'single_character'
     and cardinality(v_participant_character_ids) <> 1 then
    raise exception using
      errcode = 'P0001',
      constraint = 'qry_chat_thread_runtime_binding_participants_unavailable',
      message = 'active single-Character thread must have exactly one participant in its active content bundle';
  end if;
...
comment on function public.qry_chat_thread_runtime_binding_v1(uuid, uuid) is
'Owner-scoped active Chat thread runtime binding. Requires exactly one active primary Character and exactly one active participant for single-Character threads before returning deterministic participants.';
