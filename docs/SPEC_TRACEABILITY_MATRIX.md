...
| UC-26 | 유료 Reading/content 구매 | `COMMERCE_ENTITLEMENT`, `API_CONTRACT`, `SHARED_DOMAIN_CONTRACTS`, `COST_QUOTA_ABUSE` | §12 Commerce | COVERED-P0 (Commerce Architecture resolves `SRC-18` product→grant target and `SRC-21` event apply/aggregate authority; `P0-CM-01` Web + one-off launch rail and `P0-CM-02` PortOne V2 provider are DECIDED; launch saleability remains gated by OPEN-P0 `P0-CM-03` plus live merchant/credential/PG/channel/Production readiness) |
| UC-27 | entitlement restore | `COMMERCE_ENTITLEMENT`, `API_CONTRACT` | §12 Commerce | COVERED (Commerce Architecture resolves `SRC-18` historical source→grant target and `SRC-21` restore transition/aggregate authority; `P0-CM-01` / `P0-CM-02` are DECIDED; provider restore runtime and live Production readiness remain implementation/operational gates, not source/P0 gaps) |
...
| UC-33 | AI/Saju partial failure/retry | `API_CONTRACT`, `AI_CHARACTER_RUNTIME`, `SAJU_INTEGRATION`, `DB_DDL_MIGRATION`, `SERVER_COMMAND_TRANSACTION_SPEC.md`, `SHARED_DOMAIN_CONTRACTS`, `SOURCE_AUTHORITY_GAPS` | §7 Chat, §8 Saju, §17 Failure | COVERED-P0 / COVERED-SOURCE-GAP (`P0-AI-01` remains OPEN-P0; `P0-SA-01` calculation-only transport prerequisite is DECIDED; `SRC-09` guard metadata remains blocking; `SRC-33` prevents successful transport from being promoted to validated Product response and blocks public clarification positive validation) |
...
| purchased product → grant target authority | `COMMERCE_ENTITLEMENT`, `docs/architecture/COMMERCE_ENTITLEMENT_ARCHITECTURE_V1.md`, `docs/source-authority-gaps/SRC-18_COMMERCE_PRODUCT_ENTITLEMENT_MAPPING.md` | `SRC-18` historical gap is RESOLVED; verify immutable Product Capability Set, historical Offer/Purchase pinning, and grant-target resolution; launch saleability remains separately gated by `P0-CM-03` and live provider readiness |
| entitlement grant event transition + logical aggregate authority | `COMMERCE_ENTITLEMENT`, `SERVER_COMMAND_TRANSACTION_SPEC`, `docs/architecture/COMMERCE_ENTITLEMENT_ARCHITECTURE_V1.md`, `docs/source-authority-gaps/SRC-21_ENTITLEMENT_EVENT_APPLY_AGGREGATION.md` | `SRC-21` historical gap is RESOLVED; verify event transition/stale-order/contributor/recompute/revision/outbox semantics; verified apply/runtime completion remains an implementation gate rather than source-resolution gate |
