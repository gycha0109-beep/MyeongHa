...
| UC-26 | 유료 Reading/content 구매 | `COMMERCE_ENTITLEMENT`, `API_CONTRACT`, `SHARED_DOMAIN_CONTRACTS`, `COST_QUOTA_ABUSE` | §12 Commerce | COVERED-P0 (`SRC-18` product→grant target and `SRC-21` event apply/aggregate domain authority are RESOLVED by `COMMERCE_ENTITLEMENT_ARCHITECTURE_V1`; `P0-CM-01` Web + one-off launch rail and `P0-CM-02` PortOne V2 provider are DECIDED; launch saleability remains gated by OPEN `P0-CM-03` plus separate live merchant/credential/PG/channel/Production readiness) |
| UC-27 | entitlement restore | `COMMERCE_ENTITLEMENT`, `API_CONTRACT` | §12 Commerce | COVERED (`SRC-18` historical source→grant target and `SRC-21` restore event/aggregate domain authority are RESOLVED by `COMMERCE_ENTITLEMENT_ARCHITECTURE_V1`; `P0-CM-01` rail and `P0-CM-02` provider are DECIDED; provider restore runtime and Production readiness remain implementation/operational gates) |
...
| UC-33 | AI/Saju partial failure/retry | `API_CONTRACT`, `AI_CHARACTER_RUNTIME`, `SAJU_INTEGRATION`, `DB_DDL_MIGRATION`, `SERVER_COMMAND_TRANSACTION_SPEC.md`, `SHARED_DOMAIN_CONTRACTS`, `SOURCE_AUTHORITY_GAPS` | §7 Chat, §8 Saju, §17 Failure | COVERED-P0 / COVERED-SOURCE-GAP (`P0-AI-01` remains OPEN-P0; `P0-SA-01` calculation-only transport prerequisite is DECIDED; `SRC-09` guard metadata; `SRC-33` prevents successful transport from being promoted to validated Product response and blocks public clarification positive validation) |
...
| purchased product → grant target authority | `SOURCE_AUTHORITY_GAPS` (`SRC-18`, historical provenance), `COMMERCE_ENTITLEMENT`, `COMMERCE_ENTITLEMENT_ARCHITECTURE_V1` | current domain authority is resolved by Commerce Architecture; purchase/restore grant-target resolution remains implementation/verification work, not pending source resolution |
| entitlement grant event transition + logical aggregate authority | `SOURCE_AUTHORITY_GAPS` (`SRC-21`, historical provenance), `COMMERCE_ENTITLEMENT`, `SERVER_COMMAND_TRANSACTION_SPEC`, `COMMERCE_ENTITLEMENT_ARCHITECTURE_V1` | current event-transition/aggregate authority is resolved by Commerce Architecture; verified apply, stale ordering, valid-grant predicate, active-count, effective-expiry, revision and outbox behavior remain implementation/runtime verification work, not pending source resolution |
