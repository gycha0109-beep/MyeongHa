...
이 문서는 ERD 컬럼/PK/FK를 재창작하지 않는다. source-level schema correction이 필요하면 `SOURCE_AUTHORITY_GAPS.md`를 먼저 해결한다.
...
Relationship/episode/commerce처럼 allocator + row lock + multiple mutations가 핵심인 transition은 무리하게 trigger에 숨기지 않고 server command/procedure transaction으로 유지한다.
...
- schema version/ref를 provenance에 남김
...
그 전에는 `migration-ready/complete`라고 부르지 않는다.
