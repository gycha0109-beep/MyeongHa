# 명하 DB DDL / Migration Specification v0.5 — Source Aligned
...
Relationship/episode/commerce처럼 allocator + row lock + multiple mutations가 핵심인 transition은 무리하게 trigger에 숨기지 않고 server command/procedure transaction으로 유지한다. 단, 해당 command가 source gap으로 막혀 있으면 relational constraints만 구현하고 policy function은 만들지 않는다.
