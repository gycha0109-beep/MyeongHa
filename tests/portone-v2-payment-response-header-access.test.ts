...
type HeaderName = 'content-type' | 'content-length';

function createHarness(throwingHeader: HeaderName) {
  const observed = {
    bodyCancelCalls: 0,
    bodyReaderCalls: 0,
...
function createMalformedValueHarness(malformedHeader: HeaderName) {
  const observed = {
    bodyCancelCalls: 0,
    bodyReaderCalls: 0,
    verifiedClockCalls: 0,
  ...
  const fetchImpl: PortOneV2PaymentHttpFetchV1 = async () =>
    ({
      ...
      body: {
        ...
        getReader(): never {
          observed.bodyReaderCalls += 1;
          throw new Error('body parsing must not start after malformed header value');
        },
      },
      async text(): Promise<string> {
        throw new Error('whole-body fallback must not run');
...
describe('PortOne V2 payment response header access boundary', () => {
  ...
  it.each(['content-type', 'content-length'] as const)(
    'maps malformed %s value shape to NETWORK_FAILURE before body parsing or evidence promotion',
    async (malformedHeader) => {
      const { adapter, observed } = createMalformedValueHarness(malformedHeader);
