const readerCatalog = {
  baekheon: {
    ...
    lines: [
      '결과를 보고, 그 선택의 귀결까지 함께 보겠습니다.',
      '시기는 결론이 아니라 판단할 때 함께 봐야 할 조건입니다.',
      '선택의 근거가 보이면, 감당해야 할 결과도 조금 더 선명해집니다.',
      '읽기는 여기까지입니다. 이제 실제 선택은 당신의 몫입니다.',
    ],
  },
  seyeon: {
    name: '세연',
    ...
    lines: [
      '읽고 나서 마음이 어디에서 조금 편해지는지도 같이 볼게요.',
      '좋고 나쁨보다, 지금 나에게 필요한 시간이 어떤 쪽인지 먼저 봐요.',
      '결과가 마음을 대신 결정할 필요는 없어요. 같이 정리해보면 돼요.',
      '여기까지 읽었어요. 이제 어떤 부분이 가장 마음에 남았어요?',
    ],
  },
  yeoul: {
    name: '여울',
    ...
    lines: [
      '결과가 좋고 나쁘고보다, 지금 뭘 놓치고 있는지가 더 중요하니까요.',
      '시기가 있다는 말이 가만히 있으라는 뜻은 아니에요. 조건을 보자는 거죠.',
      '결정할 거면 근거는 알고 결정해요. 괜히 나중에 딴소리하지 말고.',
      '됐어요. 읽을 건 다 읽었어요. 이제 당신 얘기를 해봐요.',
    ],
  },
  seorin: {
    name: '서린',
    ...
    lines: [
      '한 번 지나간 선택도, 지금의 맥락에서는 다른 의미로 이어질 수 있어요.',
      '시기의 변화는 이전 기록과 나란히 놓아보면 조금 더 잘 보이기도 해요.',
      '지금의 선택이 오래 남을지, 잠시 스쳐갈지는 차분히 구분해볼게요.',
      '이번 읽기는 여기까지 남겨둘게요. 다음 이야기가 생기면 다시 이어봐요.',
    ],
  },
  rahyeon: {
    name: '라현',
    ...
    lines: [
      '답을 고르는 것보다 왜 그 답에 끌리는지가 더 재미있을 것 같네요.',
      '때가 좋다는 말에 너무 쉽게 기대진 마세요. 당신이 움직이는 이유도 봐야죠.',
      '결과보다 당신이 어떤 선택에 계속 시선을 두는지, 그쪽이 더 궁금하네요.',
      '이제 기록은 충분히 봤어요. 남은 건 당신이 어떤 답을 꺼내느냐겠죠.',
    ],
  },
  mira: {
    name: '미라',
    ...
    lines: [
      '결국 중요한 건 이걸 실제 생활에서 어떻게 써먹느냐니까요.',
      '시기 정보는 참고하면 되고, 지금 할 수 있는 건 따로 정리하면 돼요.',
      '이 정도면 판단 재료는 충분하네요. 괜히 더 복잡하게 만들 필요 없어요.',
      '읽기는 끝. 이제 현실 얘기로 돌아가죠.',
    ],
  },
  taegyeom: {
    name: '태겸',
    ...
    lines: [
      '판단할 거라면 적어도 무엇을 근거로 판단하는지는 분명해야 합니다.',
      '시기의 유불리보다 적용 범위와 한계를 먼저 확인하십시오.',
      '가능성과 확정을 혼동하지 마십시오. 읽기는 판단 재료일 뿐입니다.',
      '검토는 끝났습니다. 이제 실제 조건과 대조해보면 됩니다.',
    ],
  },
  yunho: {
    name: '윤호',
    ...
    lines: [
      '조금 천천히 봐도 괜찮습니다. 흐름을 알면 선택지가 더 선명해지니까요.',
      '시기를 하나의 조건으로 놓고 보면 지금 필요한 준비도 같이 보입니다.',
      '결론보다 기준을 남겨두는 편이 이후의 선택에도 도움이 될 겁니다.',
      '여기까지 정리해두죠. 궁금한 부분은 대화에서 더 천천히 봐도 됩니다.',
    ],
  },
  doyoon: {
    name: '도윤',
    ...
    lines: [
      '답이 뻔해 보여도, 의외로 다른 길이 하나쯤 숨어 있을 수 있잖아요.',
      '타이밍만 보고 움직이는 건 재미없죠. 지금 가진 패도 같이 봐야 하고.',
      '결과가 길을 하나로 정해주는 건 아니니까, 쓸 만한 힌트만 챙기죠.',
      '여기까지. 이제 기록 말고 진짜 당신 생각을 들어볼까요?',
    ],
  },
};
...
const requestedScope = (params.get('scope') || 'year').toLowerCase();
const scopeName = scopeLabels[requestedScope] || scopeLabels.year;
...
const characterLines = [...document.querySelectorAll('[data-reader-line]')];
...
function renderStep() {
  ...
  characterLines.forEach((element) => { element.textContent = reader.lines[stepIndex] || reader.lines[0]; });
  ...
  if (nextButton) {
    nextButton.querySelector('span:first-child')?.replaceChildren(
      document.createTextNode(stepIndex === readingSteps.length - 1 ? '대화로 이어가기' : '다음 읽기'),
    );
  }
}
