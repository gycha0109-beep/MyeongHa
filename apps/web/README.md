...
`npm run build:web` produces the Vercel static artifact in `public/`. Hall, Auth, Birth, Reading hub/detail, and My are React entries; the remaining pages continue to use their existing ES module entry points until each page is migrated. Auth keeps its existing session and Guest-promotion orchestration behind a small controller interface while React owns the form DOM. Reading keeps the existing Saju session, calculation, and route-authority controllers behind React-owned page DOM during the staged migration.
