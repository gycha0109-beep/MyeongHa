...
`npm run build:web` produces the Vercel static artifact in `public/`. Hall, Auth, and My are React entries; the remaining pages continue to use their existing ES module entry points until each page is migrated. Auth keeps its existing session and Guest-promotion orchestration behind a small controller interface while React owns the form DOM.
