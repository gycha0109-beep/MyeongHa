...
function mapPostgresError(error: unknown): never {
  ...
  switch (constraint) {
    ...
    case 'cmd_standard_reading_access_v2_additional_requires_official':
      return fail('ADDITIONAL_REQUIRES_OFFICIAL', 'Additional Reader purchase requires an existing official Reading.');
    case 'cmd_standard_reading_access_v2_initial_official_conflict':
      return fail('INITIAL_OFFICIAL_CONFLICT', 'Initial Reading purchase conflicts with an existing official Reading.');
