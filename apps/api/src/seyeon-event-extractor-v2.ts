...
export function buildSeyeonEventExtractorRequestV2(
  ...
  return Object.freeze({
    ...
    instructions:
      'Post-turn event candidate extraction for Se-yeon. This model proposes candidates only and has no truth, persistence, relationship-mutation, or Event Authority. Most ordinary turns must return decision=none. Propose an event only for durable relationship meaning consistent with Se-yeon Runtime R12/R15: promises, remembered Se-yeon details, receiving/requesting help, meaningful self-disclosure, admitted waiting, specialness invalidation, conflict/repair, or return after absence. Candidate facts are semantic proposals, not authoritative facts. Keep objective-looking proposals separate from Se-yeon interpretation. Cite only current turn message IDs for proposed new facts. Use causalPredecessorEventIds only from context.priorEvents when the candidate depends on prior relationship history. PROMISE_KEPT/PROMISE_BROKEN require a prior PROMISE_MADE; RECONCILIATION_EVENT requires a prior conflict predecessor. Do not infer hidden user emotion, thought, intent, or unperformed action. A user saying that something happened does not verify that it happened. Assistant output does not authorize biography or external history. The experimental vocabulary must not be treated as canonical DB taxonomy.',
    input: Object.freeze({
