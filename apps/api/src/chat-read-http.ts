...
export const CHAT_READ_HTTP_BINDING_V1 = Object.freeze({
  ...
  presentationConvention: 'lowercase-canonical-character-id:v1',
...
function stringValue(name: string, value: unknown): string {
  if (typeof value !== 'string' || value.trim().length === 0) throw new Error(`Chat read ${name} is invalid.`);
  return value;
}
...
function integerValue(name: string, value: unknown): number {
  ...
  if (!Number.isSafeInteger(parsed) || parsed < 0) throw new Error(`Chat read ${name} is invalid.`);
  return parsed;
}

function timestampValue(name: string, value: unknown): string {
  const date = value instanceof Date ? value : typeof value === 'string' ? new Date(value) : null;
  if (date === null || !Number.isFinite(date.getTime())) throw new Error(`Chat read ${name} is invalid.`);
...
function methodNotAllowed(): Response {
  return new Response(null, { status: 405, headers: { Allow: GET_METHOD, 'Cache-Control': NO_STORE } });
}

function parseRequest(request: Request): { threadId: string; presentationKey: string; afterSequenceNo: number } | null {
  ...
  let threadId: string;
  try { threadId = decodeURIComponent(raw); } catch { return null; }
  if (!isUuid(threadId)) return null;

  const allowed = new Set(['presentationKey', 'afterSequenceNo']);
  if ([...url.searchParams.keys()].some((key) => !allowed.has(key))) return null;
  const presentationValues = url.searchParams.getAll('presentationKey');
  if (presentationValues.length !== 1) return null;
  const presentationKey = presentationValues[0]?.trim() ?? '';
  if (!/^[a-z0-9][a-z0-9_-]{0,63}$/.test(presentationKey)) return null;
...
function mapThreadBinding(row: ThreadBindingRow, threadId: string) {
  ...
  if (returnedThreadId !== threadId || row.status !== 'active') throw new Error('Chat read thread binding is inconsistent.');
  ...
  const participants = row.participantCharacterIds.map((value) => stringValue('participant identity', value));
  return Object.freeze({
    ...
    participantCharacterIds: Object.freeze(participants),
  });
}

function resolvePresentationCharacter(participants: readonly string[], presentationKey: string): string | null {
  const matches = participants.filter((characterId) => characterId.toLowerCase() === presentationKey);
  return matches.length === 1 ? matches[0] ?? null : null;
}

function mapStreamRow(row: StreamRow) {
  if (typeof row.redacted !== 'boolean') throw new Error('Chat read redaction flag is invalid.');
  const sequenceNo = integerValue('message sequence', row.sequenceNo);
  return Object.freeze({
...
function mapRelationship(row: RelationshipRow, characterId: string) {
  ...
  if (returnedCharacterId !== characterId) throw new Error('Chat read relationship identity is inconsistent.');
  return Object.freeze({
...
async function readChatState(client: PostgresTransactionQueryV1, subjectId: string, parsed: ReturnType<typeof parseRequest> extends infer T ? Exclude<T, null> : never) {
  const bindingResult = await client.query<ThreadBindingRow>(READ_THREAD_BINDING_SQL, [subjectId, parsed.threadId]);
  if (bindingResult.rows.length !== 1 || bindingResult.rows[0] === undefined) throw new Error('Chat read thread binding row count is invalid.');
  const binding = mapThreadBinding(bindingResult.rows[0], parsed.threadId);
  const characterId = resolvePresentationCharacter(binding.participantCharacterIds, parsed.presentationKey);
  if (characterId === null) return null;

  const [streamResult, relationshipResult] = await Promise.all([
    client.query<StreamRow>(READ_STREAM_SQL, [subjectId, parsed.threadId, parsed.afterSequenceNo]),
    client.query<RelationshipRow>(READ_RELATIONSHIP_SQL, [subjectId, characterId]),
  ]);
  ...
  const latestCharacterMessage = [...messages].reverse().find((message) =>
    !message.redacted && message.senderType === 'character' && message.characterId === characterId && typeof message.bodyText === 'string' && message.bodyText.trim().length > 0,
  ) ?? null;
  const lastSequenceNo = messages.length === 0 ? parsed.afterSequenceNo : messages[messages.length - 1]?.sequenceNo ?? parsed.afterSequenceNo;
  if (relationshipResult.rows.length > 1) throw new Error('Chat read relationship row count is invalid.');
  const relationshipRow = relationshipResult.rows[0];

  return Object.freeze({
    threadId: binding.threadId,
    characterId,
    presentationKey: parsed.presentationKey,
    ...
    relationship: relationshipRow === undefined ? null : mapRelationship(relationshipRow, characterId),
  });
}

export async function handleChatReadRequestV1(input: HandleChatReadRequestInputV1): Promise<Response> {
  if (input.request.method !== GET_METHOD) return methodNotAllowed();
  const parsed = parseRequest(input.request);
  if (parsed === null) return jsonError(400, 'INVALID_REQUEST', 'request.invalid', input.requestId);
  const verifiedEvidence = await input.identityEvidenceVerifier.verifyRequestIdentity(input.request);
  if (verifiedEvidence === null) return jsonError(401, 'AUTH_REQUIRED', 'auth.required', input.requestId);

  try {
    const data = await executePostgresSubjectTransactionV1({
      pool: input.pool,
      verifiedEvidence,
      execute: ({ resolvedSubject, client }) => readChatState(client, resolvedSubject.subjectId, parsed),
    });
    if (data === null) return jsonError(404, 'NOT_FOUND', 'chat.not_found', input.requestId);
    return success(data, stringValue('request id', input.requestId), stringValue('server time', input.serverTime));
  } catch (error) {
    ...
    if (constraint !== null && (constraint.includes('_required') || constraint.includes('_valid'))) {
