...
describe('FR-15 neutral observation schema', () => {
  ...
  it('opens neutral ingestion only for an explicitly production-neutral compatible provider', () => {
    const readiness = assessNeutralObservationBundleReadinessFR15({
      bundle: fixtureBundle(),
      profile: candidateProfile,
      compatibilityReport: productionCompatibility,
    });
  ...
  it('accepts explicit unavailable observations without treating missing geometry as negative evidence', () => {
    const base = fixtureBundle();
    const limited: NeutralObservationBundleV1 = {
      ...base,
      observations: base.observations.map((entry) => entry.anchorRef === 'left_eye'
        ? {
            ...entry,
            availability: 'unavailable' as const,
            geometry: undefined,
            quality: { visibility: 'not_visible' as const, confidence: null, reasons: ['occluded'] },
          }
        : entry),
    };
    const readiness = assessNeutralObservationBundleReadinessFR15({
  ...
  it('rejects provider-version drift and raw/biometric persistence', () => {
    ...
    expect(() => validateNeutralObservationBundleFR15({
      ...base,
      providerContractVersion: 'facelab-neutral-v2',
    }, candidateProfile)).toThrow(/exact pinned providerContractVersion/u);
  ...
  it('rejects identity/provider-specific fields smuggled into the neutral payload', () => {
    const base = fixtureBundle();
    const forged = {
      ...base,
      faceEmbedding: [0.1, 0.2],
    } as NeutralObservationBundleV1;
    expect(() => validateNeutralObservationBundleFR15(forged, candidateProfile)).toThrow(/unauthorized field/u);
  });

  it('issues only neutral-observation authority and rejects forged issued artifacts', () => {
    const issued = issueNeutralObservationArtifactFR15({
      bundle: fixtureBundle(),
      profile: candidateProfile,
      compatibilityReport: productionCompatibility,
    });
