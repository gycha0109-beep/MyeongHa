...
[[ "$("${psql_base[@]}" -Atc "select count(*) from information_schema.tables where table_schema='public' and table_type='BASE TABLE';")" == "63" ]] || fail "guest promotion slice changed public table catalog"
pass "guest promotion command PUBLIC EXECUTE remains revoked and public table catalog remains 63"
