...
insert into public.content_bundles(
  ...
  'test://account-delete','content-bundle-v1','0.0.1-dev','sha256:account-delete-assets','cue-v1','{}',clock_timestamp()
);
...
insert into public.conversation_threads(
  ...
  ('b5000000-0000-0000-0000-000000000002','b2000000-0000-0000-0000-000000000001','system','active','post-delete-probe',
   'b3100000-0000-0000-0000-000000000001','b3000000-0000-0000-0000-000000000001',0,1,clock_timestamp(),clock_timestamp(),null);

select * from public.cmd_receive_chat_turn_v1(
  'b2000000-0000-0000-0000-000000000001','b5000000-0000-0000-0000-000000000001',
  'pre-delete-turn','sha256:pre-delete-turn','chat-request-v1','{}',
...
select * from public.cmd_create_reading_session_v1(
  ...
  'pre-delete-reading','sha256:pre-delete-reading','reading-request-v1','{}',
...
insert into public.share_artifacts(
  ...
  'b6100000-0000-0000-0000-000000000001','sha256:account-delete-share-token','share-v1','{}',
...
result=$("${psql_base[@]}" -Atc "select deletion_job_id||'|'||deletion_job_status||'|'||case when replayed then '1' else '0' end from public.cmd_start_account_deletion_v1('b2000000-0000-0000-0000-000000000001','b8000000-0000-0000-0000-000000000001','account-delete-a','b8100000-0000-0000-0000-000000000001');")
[[ "$result" == "b8000000-0000-0000-0000-000000000001|running|0" ]] || fail "account deletion start result mismatch: $result"

state=$("${psql_base[@]}" -Atc "select status||'|'||case when auth_user_id='b1000000-0000-0000-0000-000000000001' then '1' else '0' end from public.subjects where id='b2000000-0000-0000-0000-000000000001';")
[[ "$state" == "deletion_pending|1" ]] || fail "subject deletion_pending/auth mapping mismatch: $state"

job=$("${psql_base[@]}" -Atc "select scope||'|'||status||'|'||case when started_at is not null then '1' else '0' end||'|'||case when completed_at is null then '1' else '0' end from public.data_deletion_jobs where id='b8000000-0000-0000-0000-000000000001';")
[[ "$job" == "account|running|1|1" ]] || fail "account deletion job shape mismatch: $job"

share=$("${psql_base[@]}" -Atc "select status||'|'||case when revoked_at is not null then '1' else '0' end from public.share_artifacts where id='b7000000-0000-0000-0000-000000000001';")
[[ "$share" == "revoked|1" ]] || fail "share artifact revoke mismatch: $share"

device=$("${psql_base[@]}" -Atc "select case when revoked_at is not null then '1' else '0' end||'|'||case when push_token_encrypted='ciphertext-a' then '1' else '0' end from public.device_installations where id='b7100000-0000-0000-0000-000000000001';")
[[ "$device" == "1|1" ]] || fail "device revoke/destructive-boundary mismatch: $device"
...
provenance=$("${psql_base[@]}" -Atc "select case when exists(select 1 from public.chat_turns where id='b5100000-0000-0000-0000-000000000001') then '1' else '0' end||'|'||case when exists(select 1 from public.readings where id='b6100000-0000-0000-0000-000000000001') then '1' else '0' end;")
[[ "$provenance" == "1|1" ]] || fail "deletion start rewrote existing chat/reading provenance: $provenance"
...
replay=$("${psql_base[@]}" -Atc "select deletion_job_id||'|'||deletion_job_status||'|'||case when replayed then '1' else '0' end from public.cmd_start_account_deletion_v1('b2000000-0000-0000-0000-000000000001','b8000000-0000-0000-0000-000000000099','account-delete-a','b8100000-0000-0000-0000-000000000099');")
[[ "$replay" == "b8000000-0000-0000-0000-000000000001|running|1" ]] || fail "account deletion replay mismatch: $replay"
...
rollback_state=$("${psql_base[@]}" -Atc "select s.status||'|'||case when di.revoked_at is null then '1' else '0' end||'|'||n.status||'|'||case when not exists(select 1 from public.data_deletion_jobs dj where dj.id='b8000000-0000-0000-0000-000000000002') then '1' else '0' end from public.subjects s join public.device_installations di on di.subject_id=s.id join public.notifications n on n.subject_id=s.id where s.id='b2000000-0000-0000-0000-000000000002' and di.id='b7100000-0000-0000-0000-000000000002' and n.id='b7200000-0000-0000-0000-000000000011';")
[[ "$rollback_state" == "active|1|queued|1" ]] || fail "late outbox failure left partial deletion state: $rollback_state"
...
(
  "${psql_base[@]}" -Atc "begin; select deletion_job_id||'|'||case when replayed then '1' else '0' end from public.cmd_start_account_deletion_v1('b2000000-0000-0000-0000-000000000003','b8000000-0000-0000-0000-000000000003','account-delete-c','b8100000-0000-0000-0000-000000000003'); select pg_sleep(0.4); commit;" > /tmp/account-delete-c-1.out 2>&1
) & p1=$!
sleep 0.08
(
  "${psql_base[@]}" -Atc "select deletion_job_id||'|'||case when replayed then '1' else '0' end from public.cmd_start_account_deletion_v1('b2000000-0000-0000-0000-000000000003','b8000000-0000-0000-0000-000000000013','account-delete-c','b8100000-0000-0000-0000-000000000013');" > /tmp/account-delete-c-2.out 2>&1
) & p2=$!
wait $p1
wait $p2
race_c=$(cat /tmp/account-delete-c-1.out /tmp/account-delete-c-2.out)
[[ "$race_c" == *"b8000000-0000-0000-0000-000000000003|0"* ]] || { cat /tmp/account-delete-c-1.out /tmp/account-delete-c-2.out >&2; fail "same-key race missing winner"; }
[[ "$race_c" == *"b8000000-0000-0000-0000-000000000003|1"* ]] || { cat /tmp/account-delete-c-1.out /tmp/account-delete-c-2.out >&2; fail "same-key race missing replay"; }
...
[[ "$("${psql_base[@]}" -Atc "select count(*) from information_schema.tables where table_schema='public' and table_type='BASE TABLE';")" == "62" ]] || fail "account deletion slice changed public table catalog"
pass "account deletion command PUBLIC EXECUTE remains revoked and public table catalog remains 62"
