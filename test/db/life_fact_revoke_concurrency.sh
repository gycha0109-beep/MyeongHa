...
preserved=$("${psql_base[@]}" -Atc "select (select count(*) from public.life_facts where id='73000000-0000-0000-0000-000000000001' and fact_type='employment_status' and schema_version='life-fact-v1' and value_jsonb ->> 'value' = 'employed' and source_kind='user_explicit')::text||'|'||(select count(*) from public.record_access_grants where life_fact_id='73000000-0000-0000-0000-000000000001' and revoked_at is null)::text||'|'||(select count(*) from public.life_facts where id='73000000-0000-0000-0000-000000000003' and revoked_at is null)::text||'|'||(select count(*) from public.life_facts where subject_id='72000000-0000-0000-0000-000000000002' and revoked_at is null)::text;")
...
pass "Life Fact revoke command PUBLIC EXECUTE remains revoked and public table catalog remains 60"
