...
member=$("${psql_quiet[@]}" -F '|' -c "begin; set local role myeongha_api_executor; select * from public.begin_member_subject_context_v1('51000000-0000-0000-0000-000000000001'); select subject_id,subject_kind,subject_status,display_name,locale,timezone,onboarding_state,to_char(profile_updated_at at time zone 'UTC','YYYY-MM-DD HH24:MI:SS') from public.qry_subject_profile_current_v1('51100000-0000-0000-0000-000000000001'); rollback;" | tail -n 1)
...
pending=$("${psql_quiet[@]}" -F '|' -c "begin; set local role myeongha_api_executor; select * from public.begin_member_subject_context_v1('51000000-0000-0000-0000-000000000002'); select subject_kind,subject_status,display_name,to_char(profile_updated_at at time zone 'UTC','YYYY-MM-DD HH24:MI:SS') from public.qry_subject_profile_current_v1('51100000-0000-0000-0000-000000000002'); rollback;" | tail -n 1)
...
guest=$("${psql_quiet[@]}" -F '|' -c "begin; set local role myeongha_api_executor; select * from public.begin_guest_subject_context_v1('hmac-sha256:k-test:guest-current'); select subject_kind,subject_status,coalesce(display_name,'<null>'),coalesce(locale,'<null>'),coalesce(timezone,'<null>'),coalesce(onboarding_state,'<null>'),coalesce(profile_updated_at::text,'<null>') from public.qry_subject_profile_current_v1('51100000-0000-0000-0000-000000000003'); rollback;" | tail -n 1)
...
projection_json=$("${psql_quiet[@]}" -c "begin; set local role myeongha_api_executor; select * from public.begin_member_subject_context_v1('51000000-0000-0000-0000-000000000001'); select row_to_json(q)::text from public.qry_subject_profile_current_v1('51100000-0000-0000-0000-000000000001') q; rollback;" | tail -n 1)
...
"${psql_quiet[@]}" -c "begin; set local role myeongha_api_executor; select * from public.begin_member_subject_context_v1('51000000-0000-0000-0000-000000000001'); select count(*) from public.qry_subject_profile_current_v1('51100000-0000-0000-0000-000000000001'); rollback;" >/dev/null
...
no_context_count=$("${psql_quiet[@]}" -c "begin; set local role myeongha_api_executor; select count(id) from public.subjects; rollback;" | tail -n 1)
...
expect_fail "profile query without trusted context is denied" "trusted MyeongHa subject execution context is required" "begin; set local role myeongha_api_executor; select * from public.qry_subject_profile_current_v1('51100000-0000-0000-0000-000000000001');"
...
expect_fail "unknown member auth identity cannot establish context" "verified member identity does not resolve to a current canonical subject" "begin; set local role myeongha_api_executor; select * from public.begin_member_subject_context_v1('51000000-0000-0000-0000-000000000099');"
expect_fail "unknown guest verifier cannot establish context" "verified guest identity does not resolve to an active canonical guest subject" "begin; set local role myeongha_api_executor; select * from public.begin_guest_subject_context_v1('hmac-sha256:k-test:unknown');"
expect_fail "member resolver requires verified auth identity" "verified member authentication identity is required" "begin; set local role myeongha_api_executor; select * from public.begin_member_subject_context_v1(null);"
expect_fail "guest resolver requires verified fingerprint" "verified guest credential fingerprint is required" "begin; set local role myeongha_api_executor; select * from public.begin_guest_subject_context_v1('');"
...
expect_fail "API role cannot read auth mapping column" "permission denied" "begin; set local role myeongha_api_executor; select auth_user_id from public.subjects limit 1;"
expect_fail "API role cannot mutate Subject authority directly" "permission denied" "begin; set local role myeongha_api_executor; update public.subjects set updated_at=clock_timestamp() where id='51100000-0000-0000-0000-000000000001';"
...
[[ "$("${psql_quiet[@]}" -c "select count(*) from information_schema.tables where table_schema='public' and table_type='BASE TABLE';")" == '63' ]] || fail "public table catalog changed"
