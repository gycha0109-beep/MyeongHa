...
grep -Fq "supabase db push --include-all --dry-run" "$workflow" || fail "production include-all dry-run gate is missing"
grep -Fq "supabase db push --include-all" "$workflow" || fail "production include-all migration push is missing"

repair_line=$(grep -nF "supabase migration repair 20260902193252 --status reverted" "$workflow" | head -n1 | cut -d: -f1)
dry_run_line=$(grep -nF "supabase db push --include-all --dry-run" "$workflow" | head -n1 | cut -d: -f1)
[[ "$repair_line" -lt "$dry_run_line" ]] || fail "duplicate repair must run before production db push include-all dry-run"

if grep -Eq '^\s*pull_request:' "$workflow"; then
  fail "Supabase production migration workflow must not deploy from pull_request"
fi

pass "known duplicate migration history is repaired before include-all db push"
