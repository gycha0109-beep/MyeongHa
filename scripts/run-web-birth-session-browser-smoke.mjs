...
function resetRequests() {
  ...
  requests.current = 0;
  requests.create = 0;
  requests.lastCurrentAuthorization = null;
  requests.lastCreateAuthorization = null;
}
...
function envelope(data) {
  return JSON.stringify({
    ...
    meta: {
      ...
      requestId: 'birth-session-browser-smoke',
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/session/bootstrap' && req.method === 'POST') {
        ...
        res.end(envelope({
          kind: 'guest',
          guestSession: { bearerToken: 'fresh-guest-bearer' },
        }));
        return;
      }
...
async function navigate(client, origin) {
  const result = await client.send('Page.navigate', { url: `${origin}/birth.html` });
  assert(!result.errorText, `Navigation failed: ${result.errorText}`);
  const deadline = Date.now() + 10_000;
  while (Date.now() < deadline) {
    if (await client.evaluate(`document.readyState === 'complete' && location.pathname === '/birth.html'`)) return;
    await sleep(50);
  }
  throw new Error('Timed out waiting for Birth page');
}
...
async function setStorage(client, setup) {
...
const profile = await mkdtemp(join(tmpdir(), 'myeongha-birth-session-browser-smoke-'));
...
try {
  ...
  // Fresh guest: bootstrap exactly once, preflight null, then create with the same bearer.
  resetRequests();
  ...
  const storedFreshGuest = await client.evaluate(`sessionStorage.getItem('myeongha.guestBearer.v1')`);
  assert(storedFreshGuest === 'fresh-guest-bearer', 'fresh guest bearer was not stored');
  ...
  // Existing guest: no new bootstrap, same opaque bearer for read and create.
  resetRequests();
  await setStorage(client, `sessionStorage.setItem('myeongha.guestBearer.v1', 'existing-guest-bearer');`);
  await navigate(client, origin);
  ...
  // Member with a current Birth Profile: create UI is closed and even a synthetic submit cannot POST.
  resetRequests();
  await setStorage(client, `localStorage.setItem('myeongha.memberSession.v1', JSON.stringify({
  ...
  // Expired member session: refresh and preserve member bearer path without guest bootstrap.
  resetRequests();
  await setStorage(client, `localStorage.setItem('myeongha.memberSession.v1', JSON.stringify({
  ...
  // Unauthorized current read: fail closed, no form and no create.
  resetRequests();
  await setStorage(client, `sessionStorage.setItem('myeongha.guestBearer.v1', 'unauthorized-guest-bearer');`);
  await navigate(client, origin);
  await waitFor(client, `document.querySelector('#birth-status')?.innerText.includes('새 명식록 생성을 허용하지 않습니다.')`, 'unauthorized state did not fail closed');
  assert(await client.evaluate(`document.querySelector('#birth-form')?.hidden === true`), 'unauthorized state exposed create form');
  assert(requests.create === 0, 'unauthorized state issued create POST');

  // Malformed authoritative envelope: fail closed.
  resetRequests();
  await setStorage(client, `sessionStorage.setItem('myeongha.guestBearer.v1', 'malformed-guest-bearer');`);
  await navigate(client, origin);
  await waitFor(client, `document.querySelector('#birth-status')?.innerText.includes('중복 생성을 막기 위해 새 명식록 입력을 잠갔습니다.')`, 'malformed current envelope did not fail closed');
  assert(await client.evaluate(`document.querySelector('#birth-form')?.hidden === true`), 'malformed current envelope exposed create form');

  // 390px mobile: empty authoritative state opens a bounded form without horizontal overflow.
  resetRequests();
