...
const entryKey = `${entryPrefix}removal-failure-authority`;
...
const email = 'confirmation-removal-failure@example.com';
const password = 'browser-password-12345';
const guestBearer = 'guest-confirmation-removal-failure';
const memberAccess = 'confirmation-removal.member.signature';
...
function envelope(data) {
  ...
  return {
    ...
    data,
    meta: {
      apiContractVersion: 'auth-confirmation-handoff-removal-failure-browser-v1',
      requestId: `auth-confirmation-removal-failure-${requestNo}`,
      ...
      if (pathname === '/api/auth/sign-in' && req.method === 'POST') {
        ...
        sendJson(res, 200, envelope({
          ...
          session: {
            ...
            refreshToken: 'refresh-confirmation-removal-failure',
...
assert(authPageSource.includes('WEB_AUTH_CONFIRMATION_HANDOFF_CLEAR_FAILED'), 'auth-page.js lacks confirmation handoff clear-failure authority');

const { server, origin } = await serve();
const profile = await mkdtemp(join(tmpdir(), 'myeongha-auth-confirmation-handoff-removal-failure-browser-'));
...
try {
  ...
  await client.send('Page.addScriptToEvaluateOnNewDocument', {
    source: `(() => {
      ...
      globalThis.__myeonghaFailConfirmationHandoffRemoval = true;
      Storage.prototype.removeItem = function(key) {
        if (globalThis.__myeonghaFailConfirmationHandoffRemoval && key === ${JSON.stringify(entryKey)}) {
          throw new Error('confirmation handoff removal blocked');
        }
  ...
  await waitFor(
    client,
    `document.querySelector('#auth-status')?.textContent?.includes('게스트 계정 연결 기록을 정리하지 못했습니다.') === true && document.querySelector('#auth-submit')?.disabled === false`,
    'Confirmation handoff removal failure did not remain on auth page',
  );
  ...
  assert(await client.evaluate(`location.pathname === '/auth.html'`) === true, 'Removal failure redirected away from auth page');
  ...
  assert(preserved.marker === '1', 'Journal marker was lost during removal failure');
  assert(preserved.entry === JSON.stringify(entry), 'Authoritative journal entry was lost during failed removal');
  ...
  assert(preserved.pending === null, 'Unexpected pending Guest mutation after promotion cleanup failure');

  await client.evaluate(`globalThis.__myeonghaFailConfirmationHandoffRemoval = false`);
  ...
  assert(recovered.entry === null, 'Rejected stale confirmation handoff entry was not consumed after recovery');
