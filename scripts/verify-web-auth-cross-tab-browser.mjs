...
const artifactPath = join(artifactDir, 'web-auth-cross-tab-browser-smoke.json');
const testIdentity = Object.freeze({
  id: '55555555-5555-4555-8555-555555555555',
  email: 'cross-tab-member@example.com',
...
function successEnvelope(data) {
  ...
  return {
    ...
    data,
    meta: {
      apiContractVersion: 'browser-auth-cross-tab-v1',
      requestId: `web-auth-cross-tab-${apiRequestCount}`,
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (pathname === '/api/session/bootstrap' && req.method === 'POST') {
        ...
        sendJson(res, 200, successEnvelope({
          kind: 'guest',
          guestSession: { bearerToken: 'unexpected-cross-tab-bootstrap' },
        }));
        return;
      }
...
const profile = await mkdtemp(join(tmpdir(), 'myeongha-auth-cross-tab-browser-'));
...
try {
  ...
  await waitFor(
    tabB,
    `document.querySelector('.product-profile')?.dataset.authState === 'guest' && sessionStorage.getItem('myeongha.guestBearer.v1') === ${JSON.stringify(tabBGuestBearer)}`,
  ...
  await waitFor(
    ...
    'Tab B did not converge its compatibility bearer after Tab A Member sign-in',
  );
  const memberTabB = await authSnapshot(tabB);
  const memberBearer = await activeBearerSnapshot(tabB);
  assert(memberTabB.authState === 'member' && memberTabB.authLabel === '마이 페이지', 'Tab B did not visibly converge to Member');
  assert(memberTabB.memberUserId === testIdentity.id, 'Tab B converged to the wrong Member identity');
  assert(memberTabB.activeBearer === memberAccessToken, 'Tab B retained its Guest bearer after cross-tab Member sign-in');
  assert(memberTabB.pendingGuest === tabBGuestBearer, 'Tab B did not preserve its pre-login Guest bearer');
  ...
  await waitFor(
    ...
    'Tab A sign-out did not remove the shared Member session',
  );
  ...
  await waitFor(
    ...
    'Tab B did not scrub the stale Member credential and restore its Guest bearer after Tab A sign-out',
  );
  const signedOutTabB = await authSnapshot(tabB);
  const guestBearer = await activeBearerSnapshot(tabB);
  assert(signedOutTabB.authState === 'guest' && signedOutTabB.authLabel === '로그인', 'Tab B did not visibly converge to Guest after cross-tab sign-out');
  assert(signedOutTabB.memberAccessToken === null && signedOutTabB.memberUserId === null, 'Tab B retained Member identity metadata after sign-out');
  assert(signedOutTabB.activeBearer === tabBGuestBearer, 'Tab B did not restore its own Guest bearer after sign-out');
  assert(signedOutTabB.pendingGuest === null, 'Tab B left the restored Guest bearer duplicated in the pending slot');
  ...
  console.log('MyeongHa_WEB_AUTH_CROSS_TAB_BROWSER_PASS');
} catch (error) {
  await mkdir(artifactDir, { recursive: true });
  await writeFile(artifactPath, `${JSON.stringify({
    status: 'MyeongHa_WEB_AUTH_CROSS_TAB_BROWSER_FAIL',
