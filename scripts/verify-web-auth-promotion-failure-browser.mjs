...
function successEnvelope(data) {
  ...
  return {
    ...
    data,
    meta: {
      apiContractVersion: 'browser-auth-promotion-failure-v1',
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (pathname === '/api/auth/promote-guest' && req.method === 'POST') {
        ...
        assert(authorization === `Bearer ${expectedSession.accessToken}`, `Promotion used unexpected Member bearer on login ${signInCount}`);
        assert(promotedGuest === guestBearer, `Promotion lost the preserved Guest bearer on login ${signInCount}`);
...
async function prepareAuthAttempt(client) {
  ...
  await client.evaluate(`(() => {
    ...
    record();
    new MutationObserver(record).observe(status, { childList: true, characterData: true, subtree: true, attributes: true, attributeFilter: ['class'] });
  })()`);
}
...
async function readHallAuthority(client) {
...
function assertPreservedAttempt(state, attempt, label) {
  ...
  assert(
    !state.trace.some((entry) => entry.className.includes('is-error') && entry.text.includes('인증을 완료하지 못했습니다')),
    `${label} surfaced an authentication failure after Member authentication had already succeeded`,
  );
}
...
try {
  ...
  const afterNetworkFailure = await readHallAuthority(client);
  ...
  const afterUpstreamFailure = await readHallAuthority(client);
  ...
  const afterRecovery = await readHallAuthority(client);
  const expectedRecoverySession = sessionFor(3);
  assert(afterRecovery.profileState === 'member', 'Healthy promotion recovery lost Member state');
  assert(afterRecovery.memberId === testIdentity.id, 'Healthy promotion recovery changed Member identity');
  assert(afterRecovery.memberAccessToken === expectedRecoverySession.accessToken, 'Healthy promotion recovery stored unexpected Member token');
  assert(afterRecovery.activeBearer === expectedRecoverySession.accessToken, 'Healthy promotion recovery did not keep Member bearer active');
  assert(afterRecovery.pendingGuest === null, 'Healthy promotion recovery did not clear the promoted Guest');
  assert(
  ...
  assert(signIns.length === 3, `Expected three successful Member sign-ins, received ${signIns.length}`);
  assert(promotions.length === 3, `Expected three logical promotion outcomes, received ${promotions.length}`);
  ...
  assert(promotions[2].outcome === 'success', 'Third promotion outcome was not healthy recovery');
