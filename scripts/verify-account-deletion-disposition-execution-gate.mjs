...
if (!/^\|\s*`P0-PR-01`\s*\|[^|\n]*\|\s*\*\*DECIDED\*\*\s*\|/m.test(decisions)) {
  fail('P0-PR-01 must be DECIDED in the decision register while the historical candidate stays OPEN-P0');
}
...
console.log(
  'Account deletion disposition historical gate PASS: 47/47 tables and 106/106 edges remain covered by the immutable pre-approval candidate, conflict drift fails closed, and SQL generation remains absent.',
);
