...
function validateMessages(value, expectedCharacterId) {
  ...
  for (const [index, rawMessage] of messages.entries()) {
    ...
    const sequenceNo = requireNonNegativeInteger(
      `Production Chat message ${index} sequence`,
      message.sequenceNo,
    );
    ...
    const characterId = requireNullableString(
      `Production Chat message ${index} character identity`,
      message.characterId,
    );
...
const reportedLastSequenceNo = requireNonNegativeInteger(
  'Production Chat last sequence',
  data.lastSequenceNo,
);
