...
if (!sameJson(counts, { DELETE: 35, ANONYMIZE: 4, RETAIN: 8 })) {
  fail('approved 35/4/8 disposition split drifted: ' + JSON.stringify(counts));
}
...
if (plan.stepCount !== 47 || plan.destructiveSqlGenerated !== false || plan.sql !== null) {
  fail('approved structured plan must cover 47 tables and remain non-SQL');
}
...
console.log(
  'Approved account deletion policy PASS: P0-PR-01 is DECIDED, 47/47 tables map to DELETE 35 / ANONYMIZE 4 / RETAIN 8, 30 mixed FK edges are explicitly planned, retained Commerce uses calendar P5Y, structured plan generation is allowed, and destructive SQL / privacy reconciliation / DR promotion remain blocked.',
);
